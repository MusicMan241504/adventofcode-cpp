...
int main() {
	...
	while (!pq.empty()) {
		...
		if (maze[newY][newX][n->dir]->isPath) { // if is path
			...
			if (newCost <= maze[newY][newX][n->dir]->cost) {
				if (newCost == maze[newY][newX][n->dir]->cost) {
					maze[newY][newX][n->dir]->pre.push_back(n);
				} else {
					maze[newY][newX][n->dir]->pre = {n};
				}
		...
		if (newCost <= maze[n->y][n->x][(n->dir+1)%4]->cost) {
			if (newCost == maze[n->y][n->x][(n->dir+1)%4]->cost) {
				maze[n->y][n->x][(n->dir+1)%4]->pre.push_back(n);
			} else {
				maze[n->y][n->x][(n->dir+1)%4]->pre = {n};
			}
		...
		if (newCost <= maze[n->y][n->x][(n->dir+3)%4]->cost) {
			if (newCost == maze[n->y][n->x][(n->dir+3)%4]->cost) {
				maze[n->y][n->x][(n->dir+3)%4]->pre.push_back(n);
			} else {
				maze[n->y][n->x][(n->dir+3)%4]->pre = {n};
			}
	...
	std::vector<std::vector<bool>> isInAnyPath(maze.size(), std::vector<bool>(maze[0].size()));

	while(!nodes.empty()) {
		auto n{nodes.front()};
		nodes.pop_front();
		isInAnyPath[n->y][n->x] = 1;
		while(!n->pre.empty()) {
			nodes.push_back(n->pre.front());
			n->pre.pop_front();
		}
	}

	// count squares in isInAnyPath
	int squareCount{0};
	for (const auto& row : isInAnyPath) {
