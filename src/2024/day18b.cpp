...
void resetCosts(std::vector<std::vector<Node*>>& arr) {
	for (auto& row : arr) {
		for (auto& e : row) {
			e->d = 999999999;
		}
...
int main() {
	...
	std::string line;
	bool isPossiblePath{1};
	for (size_t i{0}; isPossiblePath && std::getline(f,line); i++) {
		if (line == "") {
			break;
		}
		...
		// get path distance
		if (grid[grid.size()-1][grid[0].size()-1]->d == 999999999) {
			isPossiblePath = 0;
			std::cout << x << ',' << y << '\n';
		}
